...
public class Decoder
{
    ...
    private static bool _decode = true;
    private static bool _stdout = true;
    private static int _initialcodepage = 850;
    private static bool _initialkanjion;
    private static byte _initialIcs;
    ...
    public static string DecodeByteArrayToText(byte[] escPosData, string merchantId, Settings? settings)
    {
        ...
        for (var i = 0; i < escPosList.Count; i++)
        {
            ...
            if (item.cmdtype is EscPosCmdType.PrintAndLineFeed or EscPosCmdType.PrintAndCarriageReturnLineFeed
                or EscPosCmdType.PrintAndCarriageReturn)
            {
                result += "\n";
                continue;
            }

            if (Regex.Match(BitConverter.ToString(item.cmddata), @"[^\u0021-\u007E]").Success) continue;

            if (!string.IsNullOrEmpty(item.paramdetail)) result += item.paramdetail;
    ...
    private static List<EscPosCmd> ConvertStrings(List<EscPosCmd> escPosCmds)
    {
        ...
        try
        {
            ...
            if (prtIcs != 0) prtreplaced = EscPosDecoder.s_StringESCRICS[(byte)prtIcs];
            ...
            switch (item.cmdtype)
            {
                ...
                case EscPosCmdType.PrtPrintables:
                    try
                    {
                        ...
                        item.paramdetail = string.Join("",
                            prtIcs == 0
                                ? listwork
                                : listwork.Select(s => prtreplaced.ContainsKey(s) ? prtreplaced[s] : s));
                    }
                    catch
                    {
                    }
                ...
                case EscPosCmdType.VfdDisplayables:
                    try
                    {
                        ...
                        item.paramdetail = string.Join("",
                            vfdIcs == 0
                                ? listwork
                                : listwork.Select(s => vfdreplaced.ContainsKey(s) ? vfdreplaced[s] : s));
                    }
                    catch
                    {
                    }
            ...
            if (item.somebinary != null)
                switch (item.cmdtype)
                {
                    ...
                    case EscPosCmdType.VfdEscDefineUserDefinedCharacters0507:
                        try
                        {
                            var bmparray = (Bitmap[])item.somebinary;
                            var cmdtype = "_" + item.cmdtype;
                    ...
                    case EscPosCmdType.GsDefineDownloadGraphicsDataColumnDW:
                        try
                        {
                            ...
                            foreach (var bitmap in bmparray)
                            {
                                var filename = _graphicspath + "\\" + snstring + indexnumber.ToString("D3") + cmdtype +
                                               ".bmp";
                                bitmap.Save(filename, ImageFormat.Bmp);
                                indexnumber++;
                            }
                    ...
                    case EscPosCmdType.FsDefineUserDefinedKanjiCharacters1616:
                        try
                        {
                            ...
                            var bitmap = (Bitmap)item.somebinary;
                            var filename = _graphicspath + "\\" + serialnumber.ToString("D4") + "_" +
                                           code.ToString("X4") + "_" + item.cmdtype + ".bmp";
                            bitmap.Save(filename, ImageFormat.Bmp);
                            serialnumber++;
                        }
                        catch (Exception e)
                    ...
                    case EscPosCmdType.GsObsoletePrintRasterBitimage:
                        ...
                        {
                            var bitmap = (Bitmap)item.somebinary;
                            var filename = _graphicspath + "\\" + serialnumber.ToString("D4") + "_" + item.cmdtype +
                                           ".bmp";
                            bitmap.Save(filename, ImageFormat.Bmp);
                            serialnumber++;
                        }
    ...
    private static int Options(string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            switch (args[i].ToUpper())
            {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                if (!EscPosDecoder.PrtESCtCodePage.ContainsValue(_initialcodepage) &&
                                    ework.IsSingleByte) _initialcodepage = 20127;
                ...
                case "-K":
                {
                    ...
                    try
                    {
                        _initialkanjion = args[i].ToUpper() switch
                        {
                            "ON" => true,
                            "OFF" => false,
                            _ => _initialkanjion
        ...
        if (result == 0 && string.IsNullOrEmpty(_inputpath)) result = 1;
        return result;
    }
}